...
def remove_outliers_iqr(df, column):
    """
    Belirtilen sütundaki aykırı değerleri tespit eder ve bu satırları tamamen siler.
    """
    ...
    # Sadece sınırlar içindeki verileri tutuyoruz (Filtreleme)
    initial_count = len(df)
    df_filtered = df[(df[column] >= lower_bound) & (df[column] <= upper_bound)]
    
    removed_count = initial_count - len(df_filtered)
    if removed_count > 0:
        print(f"🗑️  {column} sütununda {removed_count} adet aykırı değer silindi.")
...
def run_preprocessing(input_file="data/raw/youtube_data.csv"):
    ...
    # 2. Eksik Veri Temizliği (Missing Values)
    # Kritik sütunları (izlenme, beğeni vb.) boş olan satırları atalım
    df = df.dropna(subset=['view_count', 'like_count', 'comment_count'])

    # 3. Aykırı Değer (Outlier) Temizliği
    # Hem izlenme hem de beğeni sayılarına göre temizlik yapıyoruz
